...
def html_generate(user_id, device_id, gte, lte, report_type, label,
    ...
    if report_type == '1':  # Monthly or Weekly Report
...
def generate_overview(req, report_type, gte, lte, label,
    ...
    # only gases will be appended to the list
    avg_aqi = avg_list([int(x['aqi']) for x in req.json()])
    # gives a call to avg_list() with a list which returns average of list
    # elements
    avg_gas = []
    min_gas = []
    max_gas = []
    ...
    img_lst = []  # list of images of graphs generated for every gas in daily report

    for gas in only_gases:  # loops through all gases

        all_values = [(int(x['payload']['d'][gas]),
                       int(x['payload']['d']['t']))
                      for x in req.json()]
        # appends value of gas along with time
        if report_type == '0':
            img_lst.append(chart_generate(
                device_id, [x[0] for x in all_values], all_gases[gas],
                gte, param_units[gas], report_type))
            # calls chart_generate() to generate graph image of gas
        avg_gas.append(avg_list([x[0] for x in all_values]))
        # list of average values of all the gases
        # list of maximum values of gas
        ...
        # list of minimum values of gases
    ...
    avg_tempr = str(int(avg_list([x['payload']['d']['temp']
                                  for x in req.json()]))) + '&#x2103;'  # '&#x2103' indicates ℃
    # avg_tempr is a list of average values of temperature
    avg_hum = str(int(avg_list([x['payload']['d']['hum']
                                for x in req.json()]))) + '%'
    # avg_hum is a list of average values of humidity
    ...
    # table will print 'Daily Overview' table on first page
    html_code = ''
    header = html_header(
        org, name[int(report_type)], lte, gte, gases, label, location)
    # header will work as header for the page
    # below html_code will generate first page of pdf
    ...
    f.write(html_code)
    f.close()
    # below code, will generate pdf according to the type of report
    if report_type == '0':
    ...
    if report_type == '1':
        temp_gas_data = [x['aqi']
                         for x in req.json()]  # it appends all values of aqi
        temp_gas_time = [int(x['payload']['d']['t'])
                         for x in req.json()]  # appends time
        # chart will generate column graph of AQI
        chart = chart_generate(
            device_id, temp_gas_data, 'AQI', temp_gas_time,
            'Ug/m3', report_type)
        img_lst.append(chart)  # only a single image will be appended
        chart_page = generate_gas_charts(
            device_id, img_lst, header, report_type)  # report_type is passed as arg, so that we know what type
        table_page = generate_table(req, device_id, header, report_type)
...
def generate_gas_charts(device_id, img_lst, header, report_type):
    ...
    for l in range(int(math.ceil(len(img_lst) / 3.0))):  # l=0,1,2,3... why divide by 3?

        chart_page = os.path.join(
            'static', str(device_id) + '_chart_' +
            str(int(time.time())) + '_' + str(l) + '.html')
        logger.info(chart_page)
...
>>>>>>> a1cbf20ebeb4950b1c908f9b8266c17280b2799d
